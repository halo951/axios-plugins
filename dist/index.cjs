...
const log = require('axios-logger');
...
const log__namespace = /*#__PURE__*/_interopNamespaceCompat(log);
    ...
    lifecycle: {
      preRequestTransform: {
        ...
        handler: (config, { origin, shared }) => {
          ...
          const source = axios__default.CancelToken.source();
      ...
      captureException: {
        runWhen: (reason) => reason instanceof axios.CanceledError,
...
const logger = (options = {}) => {
  if (options.config)
    log__namespace.setGlobalConfig(options.config);
  return {
    name: "logger",
    lifecycle: {
      transformRequest: {
        ...
        handler: (config) => {
          log__namespace.requestLogger(config);
          return config;
        }
      },
      postResponseTransform: {
        ...
        handler: (response) => {
          log__namespace.responseLogger(response);
          return response;
        }
      },
      captureException: {
        ...
        handler: (reason) => {
          log__namespace.errorLogger(reason);
          return reason;
        }
      }
    }
...
const sign = (options = {}) => {
  return {
    ...
    lifecycle: {
      async transformRequest(config) {
        let serializedStr;
        const data = json.klona(config.data);
        ...
        if (typeof options.salt === "object") {
          ...
          serializedStr = qs.stringify(data, { arrayFormat: "brackets" });
        } else if (typeof options.salt === "string") {
          serializedStr = qs.stringify(data, { arrayFormat: "brackets" }) + options.salt;
        } else {
          serializedStr = qs.stringify(data, { arrayFormat: "brackets" });
        }
        const sign2 = typeof options.algorithm === "function" ? options.algorithm(serializedStr) : cryptoJs.MD5(serializedStr).toString();
      ...
      preRequestTransform(config) {
        ...
        if (!navigator.sendBeacon) {
          ...
          config.adapter = async (config2) => {
            ...
            axios.toFormData(Object.assign({}, config2.data, config2.params), new FormData());
...
exports.OnlySend = onlySend$1;
exports.PathParams = pathParams$1;
